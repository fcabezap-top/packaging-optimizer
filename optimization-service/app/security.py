...
def require_manufacturer(token: str = Depends(oauth2_scheme)) -> TokenData:
    """Manufacturer only."""
    data = _decode_token(token)
    if data.role != "manufacturer":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Manufacturer role required")
    return data
