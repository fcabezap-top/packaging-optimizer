...
def _resolve(product: dict) -> dict:
    ...
    return {
        **_serialize(product),
        "family": _serialize(family) if family else {"id": product["family_id"], "name": "Unknown"},
        "subfamily": _serialize(subfamily) if subfamily else {"id": product["subfamily_id"], "name": "Unknown", "family_id": ""},
...
def add_product(product: ProductCreate):
    ...
    if not subfamilies_collection.find_one({"id": product.subfamily_id}):
